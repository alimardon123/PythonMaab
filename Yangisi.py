...
def solve_labirint(labirint):
    ...
    try:
        coor = [(labirint.index(a), a.index(b))
                for a in labirint for b in a if b == "X"]
        dest_x, dest_y = coor[0]

        start = [(labirint.index(a), a.index(b))
                 for a in labirint for b in a if b == "P"]
        a, b = start[0]
        old_a = a
        old_b = b
    ...
    def innerchap(labirint, x, y, old_x, old_y):
        try:
            ...
            if (labirint[x][y-1] != "#") and y-1 != 0 and y-1 != old_y and (y-1 > dest_y or labirint[x+1][y-1] != "#" or (x == dest_x and (labirint[x][y+1] == "#" or (labirint[x+1][y] == "#" and labirint[x-1][y] == "#" and y+1 == old_y)))):
            ...
            elif (labirint[x][y+1] != '#') and y < columns and y+1 <= dest_y and (y+1 != old_y):
